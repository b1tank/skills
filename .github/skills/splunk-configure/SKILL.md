...
description: >-
  ...
  OpenTelemetry audit report. Reads .observe/otel.md, classifies metrics into
...
- After an observability audit has generated `.observe/otel.md`
...
**When NOT to use:** If no audit report exists yet, ask the user to generate or
provide an OpenTelemetry coverage report first.
...
> No audit report found at `.observe/otel.md`. Generate or provide an
> OpenTelemetry observability coverage report first.
...
> Add the missing instrumentation, then regenerate the observability audit.
...
`.observe/detectors.md` with GenAI instrumentation prerequisites and identify
the specific missing signals that must be instrumented.
...
section. Do not generate a detector for a missing signal. Name the specific
human-readable GenAI surface and coverage area that must be added first.
...
|---------|--------------|----------------|-------------------------------|-----------|
| Token/context pressure | partial | truncation rate, token-limit errors, prompt/tool schema size, LLM-call fanout | Matching metrics are absent or only token usage exists | Instrument the named GenAI signals or owner-map them |
